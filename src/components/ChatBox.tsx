...
export function ChatBox({ isOpen, onToggle }: ChatBoxProps) {
  ...
  return (
    <>
      ...
      <Button
        onClick={onToggle}
        className="fixed top-1/2 right-4 -translate-y-1/2 rounded-full w-14 h-14 shadow-2xl bg-gradient-to-r from-rose-500 to-orange-500 hover:from-rose-600 hover:to-orange-600 backdrop-blur-md border border-white/20 z-50"
        size="icon"
      >
      ...
      {/* Chat Panel */}
      {isOpen && (
        <div className="fixed top-1/2 right-20 -translate-y-1/2 w-80 h-96 bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl flex flex-col z-40">
          {/* Header */}
